...
class Modular{
  ...
  void fillColors(){
    ...
    float d = dist(x+w/2,y+h/2,mouseX,mouseY);
    for(int i = 0 ;  i < cset.length;i++){
      // magic oneliner
      
      cset[i] = 255*(pow((noise(d/100.0+i*10.0+frameCount/10.0+((int)x^(int)y))),1.6));//+(frameCount/SPEED+i*STEP+modulars.indexOf(this)*SPREAD))*2.0-0.6);
  ...
  void regenerate(){
    ...
    img.noFill();
    for(int i = 0 ; i < w;i++){
      img.stroke(cset[i],pow(cset[i],1.12),pow(cset[i],1.13));
